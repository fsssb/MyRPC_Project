...
## 性能验证：echo 快链路并发梯度（2026-09-10）

口径声明（写简历/面试时务必同口径引用）：

```text
单机 echo 基准：128B 请求体，demo.echo 回显，不访问 DB/Redis/磁盘，
Python asyncio 客户端压测。数据验证框架自身的扩展性与瓶颈拐点，
不代表真实业务吞吐（业务含 AI 长耗时链路，见 AI 口径）。
```

**macOS 本机（PollPoller，loopback）**

| 并发 | QPS | p50(ms) | p95(ms) | p99(ms) |
| --- | --- | --- | --- | --- |
| 10 | 35505 | 0.27 | 0.36 | 0.54 |
| 50 | 35969 | 1.34 | 1.77 | 2.26 |
| 100 | 34407 | 2.77 | 3.81 | 4.74 |
| 300 | 26683 | 10.85 | 15.50 | 18.62 |
| 500 | 28823 | 16.70 | 20.94 | 24.27 |

**Linux（Docker 容器内 epoll ET，loopback）**

| 并发 | QPS | p50(ms) | p95(ms) | p99(ms) |
| --- | --- | --- | --- | --- |
| 10 | 35054 | 0.23 | 0.54 | 1.42 |
| 50 | 39652 | 1.10 | 2.76 | 3.84 |
| 100 | 39815 | 2.39 | 3.81 | 4.88 |
| 300 | 36073 | 7.96 | 9.81 | 15.69 |
| 500 | 34893 | 13.81 | 16.55 | 21.19 |

可复现命令：

```bash
./build/rpc_demo 2 12345 &
python3 scripts/bench.py --host 127.0.0.1 --port 12345 --method demo.echo \
  --expect-prefix bench- --concurrency 100 --duration 3 --timeout 20 --body-size 128
```

观测结论：
- 并发 10→100 时 QPS 稳定在 ~3.5-4w，延迟随排队线性上升（p50 0.23→2.4ms）；
- 并发 300+ QPS 略降（~2.7-3.6w）且 p99 抬升明显 —— 拐点在 ~100 并发，瓶颈为单连接请求串行读拆 + Python 客户端侧；
- AI 长耗时链路口径对比：Linux 容器内 20 并发 demo.ai = 58 QPS（含 250ms 模拟业务，**勿与 echo 口径混用**）。
```
